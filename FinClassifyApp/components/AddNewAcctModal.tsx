...
const AddNewAcctModal: React.FC<AddNewAcctModalProps> = ({ visible, onClose, accounts, onSave }) => {
  const [newAccountName, setNewAccountName] = useState('');
  const [selectedIcon, setSelectedIcon] = useState<string | null>(null);

  const accountIcons = ['money-check-alt', 'credit-card', 'piggy-bank', 'wallet', 'university'];
      ...
      <View style={styles.modalContainer}>
        <View style={styles.modalContent}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[styles.iconButton, selectedIcon === item && styles.selectedIcon]}
...
const styles = StyleSheet.create({
  ...
  iconButton: { backgroundColor: '#136207', padding: 16, borderRadius: 50, alignItems: 'center', marginHorizontal: 5 },
  selectedIcon: { backgroundColor: '#0A4D02', borderWidth: 2, borderColor: '#FFD700' },
  input: { width: '100%', padding: 10, borderWidth: 1, borderColor: '#ddd', borderRadius: 5, marginBottom: 10 },
  modalButtons: { flexDirection: 'row', justifyContent: 'space-between', width: '100%' },
  cancelButton: { flex: 1, backgroundColor: 'grey', padding: 10, margin: 5, alignItems: 'center', borderRadius: 5 },
  saveButton: { flex: 1, backgroundColor: '#136207', padding: 10, margin: 5, alignItems: 'center', borderRadius: 5 },
